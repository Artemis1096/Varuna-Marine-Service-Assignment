...
describe('Banking and Compliance Integration Tests', () => {
  ...
  describe('GET /compliance/cb', () => {
    it('should return cb_before in gCO2e with unit', async () => {
      ...
      expect(response.body.cb_before).toBeGreaterThan(0); // LNG should have positive CB
    });
  ...
  describe('POST /banking/bank', () => {
    ...
    it('should bank amount successfully and return KPIs', async () => {
      ...
      const response = await request(app)
        .post('/banking/bank')
        .send({ shipId: 'R001', year: 2024, amount_gco2eq });
    ...
    it('should return 400 with code CB_NOT_POSITIVE if CB is negative or zero', async () => {
      ...
      const response = await request(app)
        .post('/banking/bank')
        .send({ shipId: 'R002', year: 2024, amount_gco2eq: 1000000 });
  ...
  describe('POST /banking/apply', () => {
    ...
    it('should apply banked amount successfully and return KPIs', async () => {
      // First bank some amount from R001
      const cbResponse = await request(app)
        .get('/compliance/cb')
        .query({ shipId: 'R001', year: 2024 });
      ...
      // Now apply some of the banked amount to the same ship
      // Note: In the current implementation, banked amounts are per routeCode
      // So we apply to the same shipId that banked the amount
